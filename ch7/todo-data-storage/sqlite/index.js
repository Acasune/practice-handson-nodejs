...
exports.update = (id, update) => {
  ...
  values.push(id)
  return dbRun(`UPDATE todo SET ${setColumns.join()} WHERE id = ?`, values).then(({ changes }) => changes === 1 ? dbGet(`SELECT * FROM todo WHERE id = ?`, id).then(rowToTodo) : null
  )
}

exports.remove = id => dbRun(`DELETE FROM todo WHERE id =?`, id).then(({ changes }) => changes === 1 ? id : null)
